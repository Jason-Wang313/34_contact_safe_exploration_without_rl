...
and certificate expansion gives the best safety/coverage tradeoff under a
smooth-limit assumption. A v2 discontinuity stress shows the boundary: brittle
low-limit pockets increase certificate violations enough that conservative
probing wins under a high-harm safety score. The result is not a full benchmark
claim. It is a mechanism note: for contact, exploration can be organized around
certified physical reachability instead of reward learning when the certificate
assumptions are calibrated.
\end{abstract}

\paragraph{V2 hardening note.}
This version narrows the claim. Certificate-guided contact exploration is not a
free substitute for safe RL or hardware validation. It is supported only when
local contact-limit smoothness and violation harm are calibrated.
...
policy. The guard implicitly assumes that nearby contact cells have comparable
limits. When contact limits contain brittle discontinuities, the certificate can
be overconfident.
...
\subsection{V2 discontinuity stress}
The hostile reviewer attack is that the toy certificate is a hand-written
smoothness margin. We therefore add brittle low-limit pockets that break the
neighbor-smoothness assumption. Table~\ref{tab:discontinuous-contact-stress}
shows that the certificate still covers more cells, but it now causes 3974
violations. If each unsafe contact is priced at five covered-cell units,
conservative probing scores 22.726 while certificate expansion scores 22.356.
This is a negative boundary: the method needs calibrated discontinuity handling
and harm weighting.
...
robust treatment of discontinuities. The v2 stress shows that if discontinuities
are common or unsafe contacts are expensive, conservative probing can be the
better policy. The current paper should be read as a mechanism proposal, not as
a deployed exploration system.
